...
export default function EditRoleModal({ open, onClose, user, onSave }: EditRoleModalProps) {
  ...
  // Actualizar selectedRole cuando cambia el usuario
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      // TODO: Conectar con backend PUT /api/admin/users/:id/role
  ...
  return (
    ...
    >
            ...
            >
              ...
              <Box>
                <Typography
                  variant="body2"
                  sx={{
                    color: selectedRole === rol.value ? rol.color : '#F5F0F2',
                    fontWeight: 600,
                  }}
                >
                  {rol.label}
      ...
      <DialogActions sx={{ px: 3, pb: 2 }}>
        <Button onClick={onClose} sx={{ color: '#C4B0B8' }}>
          Cancelar
        </Button>
        <Button
          variant="contained"
